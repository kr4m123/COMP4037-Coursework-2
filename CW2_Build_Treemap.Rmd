...
treemap_data <- treemap_data %>%
  ...
  mutate(
    ...
    total_impact_normalise = (total_impact - min(total_impact)) / (max(total_impact) - min(total_impact)),
    
    # need to add 0.01 to prevent rectangles with 0 area
    total_impact_scaled = total_impact_normalise + 0.01,
    
    # apply a final scaling to prevent rectangles from getting too small
    total_impact_compressed = total_impact_scaled ^ 0.2
  ) %>%
  ungroup()
```
